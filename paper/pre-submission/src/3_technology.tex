...
``DID Method''はこのDIDsがどのタイプのDIDsであるかを示すもので、主にDID document（後述）の置き場所により多くの種類が存在し、代表的なものではdid:web, did:key, did:ionなどが挙げられる。
...
なお、DIDsからDID documentを引くことを``resolve''といい、本稿ではこれ以降「解決する」と呼称する。
\begin{figure}[h]
    \centering
...
\end{figure}
\subsection{選択的開示}
VCsやDIDsの文脈で重要な機能の一つが、選択的開示である。
...
選択的開示の具体的なアルゴリズムとしては、Selective Disclosure for JWTs(SD-JWT)や、ゼロ知識証明を用いたものなどが提案されている。
